// Code generated by nuzur go-code-gen. DO NOT EDIT.

package object_store

import (
	"encoding/json"
	"log"

	"github.com/gofrs/uuid"
	"github.com/nuzur/nem/core/entity/object_store_type_config"
	"github.com/nuzur/nem/enums"
	"time"

	"github.com/nuzur/nem/core/entity/mapper"
)

type ObjectStore struct {
	UUID          uuid.UUID                                      `json:"uuid"`
	Identifier    string                                         `json:"identifier"`
	Type          enums.ObjectStoreType                          `json:"type"`
	TypeConfig    object_store_type_config.ObjectStoreTypeConfig `json:"type_config"`
	Status        enums.ObjectStoreStatus                        `json:"status"`
	CreatedAt     time.Time                                      `json:"created_at"`
	UpdatedAt     time.Time                                      `json:"updated_at"`
	CreatedByUUID uuid.UUID                                      `json:"created_by_uuid"`
	UpdatedByUUID uuid.UUID                                      `json:"updated_by_uuid"`
}

func (e ObjectStore) String() string {
	res, _ := json.Marshal(e)
	return string(res)
}

func (e ObjectStore) PrimaryKeyValues() []string {
	return []string{}
}

func ObjectStoreFromJSON(data json.RawMessage) ObjectStore {
	entity := ObjectStore{}
	if data == nil {
		return entity
	}
	if len(data) == 0 {
		return entity
	}

	if err := json.Unmarshal(data, &entity); err != nil {
		if err2 := mapper.FlexibleUnmarshal(data, &entity); err2 != nil {
			log.Printf("flexible unmarshal error ObjectStoreFromJSON: %v\n", err2)
		}
	}
	return entity
}

func ObjectStoreSliceFromJSON(data json.RawMessage) []ObjectStore {
	entity := []ObjectStore{}
	if data == nil {
		return entity
	}
	if len(data) == 0 {
		return entity
	}

	if err := json.Unmarshal(data, &entity); err != nil {
		entity = []ObjectStore{}
		var rawSlice []json.RawMessage
		if err2 := json.Unmarshal(data, &rawSlice); err2 == nil {
			for _, raw := range rawSlice {
				item := ObjectStore{}
				if err3 := mapper.FlexibleUnmarshal(raw, &item); err3 != nil {
					log.Printf("flexible unmarshal error ObjectStoreSliceFromJSON item: %v\n", err3)
				}
				entity = append(entity, item)
			}
		}
	}
	return entity
}

func (e ObjectStore) ToJSON() json.RawMessage {
	res, err := json.Marshal(e)
	if err != nil {
		log.Printf("marshal error ObjectStoreToJSON: %v\n", err)
	}
	return res
}

func ObjectStoreToJSON(e ObjectStore) json.RawMessage {
	res, err := json.Marshal(e)
	if err != nil {
		log.Printf("marshal error ObjectStoreToJSON: %v\n", err)
	}
	return res
}

func ObjectStoreSliceToJSON(e []ObjectStore) json.RawMessage {
	if e == nil {
		return json.RawMessage{}
	}
	res, err := json.Marshal(e)
	if err != nil {
		log.Printf("marshal error ObjectStoreSliceToJSON: %v\n", err)
	}
	return res
}
