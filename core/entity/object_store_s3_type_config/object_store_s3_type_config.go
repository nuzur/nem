// Code generated by nuzur go-code-gen. DO NOT EDIT.

package object_store_s3_type_config

import (
	"encoding/json"
	"log"

	"github.com/guregu/null/v6"

	"github.com/nuzur/nem/core/entity/mapper"
)

type ObjectStoreS3TypeConfig struct {
	Region null.String `json:"region"`
	Key    null.String `json:"key"`
	Secret null.String `json:"secret"`
	Bucket null.String `json:"bucket"`
}

func (e ObjectStoreS3TypeConfig) String() string {
	res, _ := json.Marshal(e)
	return string(res)
}

func (e ObjectStoreS3TypeConfig) PrimaryKeyValues() []string {
	return []string{}
}

func ObjectStoreS3TypeConfigFromJSON(data json.RawMessage) ObjectStoreS3TypeConfig {
	entity := ObjectStoreS3TypeConfig{}
	if data == nil {
		return entity
	}
	if len(data) == 0 {
		return entity
	}

	if err := json.Unmarshal(data, &entity); err != nil {
		if err2 := mapper.FlexibleUnmarshal(data, &entity); err2 != nil {
			log.Printf("flexible unmarshal error ObjectStoreS3TypeConfigFromJSON: %v\n", err2)
		}
	}
	return entity
}

func ObjectStoreS3TypeConfigSliceFromJSON(data json.RawMessage) []ObjectStoreS3TypeConfig {
	entity := []ObjectStoreS3TypeConfig{}
	if data == nil {
		return entity
	}
	if len(data) == 0 {
		return entity
	}

	if err := json.Unmarshal(data, &entity); err != nil {
		entity = []ObjectStoreS3TypeConfig{}
		var rawSlice []json.RawMessage
		if err2 := json.Unmarshal(data, &rawSlice); err2 == nil {
			for _, raw := range rawSlice {
				item := ObjectStoreS3TypeConfig{}
				if err3 := mapper.FlexibleUnmarshal(raw, &item); err3 != nil {
					log.Printf("flexible unmarshal error ObjectStoreS3TypeConfigSliceFromJSON item: %v\n", err3)
				}
				entity = append(entity, item)
			}
		}
	}
	return entity
}

func (e ObjectStoreS3TypeConfig) ToJSON() json.RawMessage {
	res, err := json.Marshal(e)
	if err != nil {
		log.Printf("marshal error ObjectStoreS3TypeConfigToJSON: %v\n", err)
	}
	return res
}

func ObjectStoreS3TypeConfigToJSON(e ObjectStoreS3TypeConfig) json.RawMessage {
	res, err := json.Marshal(e)
	if err != nil {
		log.Printf("marshal error ObjectStoreS3TypeConfigToJSON: %v\n", err)
	}
	return res
}

func ObjectStoreS3TypeConfigSliceToJSON(e []ObjectStoreS3TypeConfig) json.RawMessage {
	if e == nil {
		return json.RawMessage{}
	}
	res, err := json.Marshal(e)
	if err != nil {
		log.Printf("marshal error ObjectStoreS3TypeConfigSliceToJSON: %v\n", err)
	}
	return res
}
