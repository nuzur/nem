// Code generated by nuzur go-code-gen. DO NOT EDIT.

package user_connection

import (
	"context"
	"database/sql"
	"fmt"
	main_entity "github.com/nuzur/nem/core/entity/user_connection"
	"github.com/nuzur/nem/core/module/user_connection/types"
	repogen "github.com/nuzur/nem/core/repository/gen"

	"go.uber.org/zap"
	"slices"
)

func (m *module) List(ctx context.Context,
	request types.ListRequest,
	opts ...Option) (types.ListResponse, error) {

	reqPlusOne := request
	reqPlusOne.PageSize = request.PageSize + 1

	query, err := m.repository.BuildListEntityQuery(
		ctx,
		reqPlusOne,
		main_entity.UserConnection{},
		false)
	if err != nil {

		return types.ListResponse{}, err
	}

	resolvedOpts := applyAllOptions(opts)
	cacheKey := fmt.Sprintf("ListUserConnection:%v", request)
	if !resolvedOpts.SkipCache {
		if cached, found := m.cache.Get(cacheKey); found {
			return cached.(types.ListResponse), nil
		}
	}

	v, listErr, _ := m.sg.Do(cacheKey, func() (any, error) {
		var rows *sql.Rows
		rows, err = m.repository.DB.QueryContext(ctx, query)
		if err != nil {

			m.logger.Error("error in executing query for ListUserConnection", zap.String("query", query), zap.Error(err))
			return types.ListResponse{}, err
		}
		defer rows.Close()
		var scanGetters []func(*repogen.UserConnection) any
		if len(request.GetIncludeFields()) > 0 {
			for _, f := range listFields {
				if slices.Contains(request.GetIncludeFields(), f) {
					scanGetters = append(scanGetters, listFieldRegistry[f])
				}
			}
		} else if len(request.GetExcludeFields()) > 0 {
			for _, f := range listFields {
				if !slices.Contains(request.GetExcludeFields(), f) {
					scanGetters = append(scanGetters, listFieldRegistry[f])
				}
			}
		} else {
			for _, f := range listFields {
				scanGetters = append(scanGetters, listFieldRegistry[f])
			}
		}

		var items []repogen.UserConnection
		for rows.Next() {
			var i repogen.UserConnection
			fields := make([]any, 0, len(scanGetters))
			for _, getter := range scanGetters {
				fields = append(fields, getter(&i))
			}
			if err := rows.Scan(fields...); err != nil {

				return types.ListResponse{}, err
			}
			items = append(items, i)
		}
		if err := rows.Close(); err != nil {

			return types.ListResponse{}, err
		}
		if err := rows.Err(); err != nil {

			return types.ListResponse{}, err
		}

		hasNextPage := false
		if len(items) > int(request.PageSize) {
			hasNextPage = true
			items = items[:request.PageSize]
		}

		return types.ListResponse{
			UserConnection: mapModelsToEntities(items),
			HasNextPage:    hasNextPage,
		}, nil
	})
	if listErr != nil {
		return types.ListResponse{}, listErr
	}
	listResponse := v.(types.ListResponse)
	if !resolvedOpts.SkipCache {
		m.cache.Set(cacheKey, listResponse, 0)
	}
	return listResponse, nil
}

var listFields = []string{

	"uuid",

	"user_uuid",

	"project_uuid",

	"project_version_uuid",

	"type",

	"type_config",

	"db_schema",

	"executions",

	"status",

	"created_at",

	"updated_at",
}

var listFieldRegistry = map[string]func(*repogen.UserConnection) any{

	"uuid": func(i *repogen.UserConnection) any { return &i.UUID },

	"user_uuid": func(i *repogen.UserConnection) any { return &i.UserUUID },

	"project_uuid": func(i *repogen.UserConnection) any { return &i.ProjectUUID },

	"project_version_uuid": func(i *repogen.UserConnection) any { return &i.ProjectVersionUUID },

	"type": func(i *repogen.UserConnection) any { return &i.Type },

	"type_config": func(i *repogen.UserConnection) any { return &i.TypeConfig },

	"db_schema": func(i *repogen.UserConnection) any { return &i.DbSchema },

	"executions": func(i *repogen.UserConnection) any { return &i.Executions },

	"status": func(i *repogen.UserConnection) any { return &i.Status },

	"created_at": func(i *repogen.UserConnection) any { return &i.CreatedAt },

	"updated_at": func(i *repogen.UserConnection) any { return &i.UpdatedAt },
}
