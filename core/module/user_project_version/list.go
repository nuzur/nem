...
func (m *module) List(ctx context.Context,
	...
	if err != nil {
		m.monitoring.Emit(monitoring.EmitRequest{
			ActionIdentifier: "list_user_project_version_query",
			Message:          "error in building query for ListUserProjectVersion",
	...
	if err != nil {
		m.monitoring.Emit(monitoring.EmitRequest{
			ActionIdentifier: "list_user_project_version_sort_buffer",
			Message:          "error in setting sort buffer for ListUserProjectVersion",
			EntityIdentifier: "user_project_version",
			Layer:            monitoring.RepositoryServiceLayer,
			Type:             monitoring.EmitTypeError,
			Data:             request,
	...
	if err != nil {
		m.monitoring.Emit(monitoring.EmitRequest{
			ActionIdentifier: "list_user_project_version_db",
			Message:          "error in executing query for ListUserProjectVersion",
			EntityIdentifier: "user_project_version",
			Layer:            monitoring.RepositoryServiceLayer,
			Type:             monitoring.EmitTypeError,
			Data:             request,
		...
		fmt.Printf("error in executing query for ListUserProjectVersion: %v \n %v\n", query, err)
		return types.ListResponse{}, err
	}
	defer rows.Close()
	...
	if err := rows.Close(); err != nil {
		m.monitoring.Emit(monitoring.EmitRequest{
			ActionIdentifier: "list_user_project_version_rows",
			Message:          "error closing rows for ListUserProjectVersion",
			EntityIdentifier: "user_project_version",
			Layer:            monitoring.ModuleServiceLayer,
			Type:             monitoring.EmitTypeError,
			Data:             request,
	...
	if err := rows.Err(); err != nil {
		m.monitoring.Emit(monitoring.EmitRequest{
			ActionIdentifier: "list_user_project_version_rows",
			Message:          "error in rows for ListUserProjectVersion",
			EntityIdentifier: "user_project_version",
			Layer:            monitoring.ModuleServiceLayer,
			Type:             monitoring.EmitTypeError,
			Data:             request,
	...
	if err != nil {
		m.monitoring.Emit(monitoring.EmitRequest{
			ActionIdentifier: "list_user_project_version_next",
			Message:          "error determining if list has next page for ListUserProjectVersion",
			EntityIdentifier: "user_project_version",
			Layer:            monitoring.ModuleServiceLayer,
			Type:             monitoring.EmitTypeError,
			Data:             request,
	...
	m.monitoring.Emit(monitoring.EmitRequest{
		...
		Message:          "successfully handled ListUserProjectVersion",
		EntityIdentifier: "user_project_version",
		Layer:            monitoring.ModuleServiceLayer,
	...
	return types.ListResponse{
		UserProjectVersions: mapModelsToEntities(items),
...
func (m *module) ListCount(ctx context.Context,
	...
	query, err := m.repository.BuildListEntityQuery(ctx, request, main_entity.UserProjectVersion{}, true, nil, nil)
	if err != nil {
		return -1, err
	}
