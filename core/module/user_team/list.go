...
func (m *module) List(ctx context.Context,
	...
	if err != nil {
		m.monitoring.Emit(monitoring.EmitRequest{
			ActionIdentifier: "list_user_team_sort_buffer",
			Message:          "error in setting sort buffer for ListUserTeam",
			EntityIdentifier: "user_team",
			Layer:            monitoring.RepositoryServiceLayer,
			Type:             monitoring.EmitTypeError,
			Data:             request,
...
func (m *module) ListCount(ctx context.Context,
	...
	query, err := m.repository.BuildListEntityQuery(ctx, request, main_entity.UserTeam{}, true, nil, nil)
	if err != nil {
		return -1, err
	}
