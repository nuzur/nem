...
func (m *module) List(ctx context.Context,
	...
	if err != nil {
		m.monitoring.Emit(monitoring.EmitRequest{
			ActionIdentifier: "list_ai_usage_query",
			Message:          "error in building query for ListAiUsage",
	...
	if err != nil {
		m.monitoring.Emit(monitoring.EmitRequest{
			ActionIdentifier: "list_ai_usage_sort_buffer",
			Message:          "error in setting sort buffer for ListAiUsage",
			EntityIdentifier: "ai_usage",
			Layer:            monitoring.RepositoryServiceLayer,
			Type:             monitoring.EmitTypeError,
			Data:             request,
	...
	if err != nil {
		m.monitoring.Emit(monitoring.EmitRequest{
			ActionIdentifier: "list_ai_usage_db",
			Message:          "error in executing query for ListAiUsage",
			EntityIdentifier: "ai_usage",
			Layer:            monitoring.RepositoryServiceLayer,
			Type:             monitoring.EmitTypeError,
			Data:             request,
		...
		fmt.Printf("error in executing query for ListAiUsage: %v \n %v\n", query, err)
		return types.ListResponse{}, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if len(optConfig.ListIncludeColumns) > 0 {
			if slices.Contains(optConfig.ListIncludeColumns, "user_prompt") {
				fields = append(fields, &i.UserPrompt)
			}
		} else if len(optConfig.ListExcludeColumns) > 0 {
			if !slices.Contains(optConfig.ListExcludeColumns, "user_prompt") {
				fields = append(fields, &i.UserPrompt)
			}
		} else {
			fields = append(fields, &i.UserPrompt)
		...
		if len(optConfig.ListIncludeColumns) > 0 {
			if slices.Contains(optConfig.ListIncludeColumns, "step") {
				fields = append(fields, &i.Step)
			}
		} else if len(optConfig.ListExcludeColumns) > 0 {
			if !slices.Contains(optConfig.ListExcludeColumns, "step") {
				fields = append(fields, &i.Step)
			}
		} else {
			fields = append(fields, &i.Step)
		...
		if len(optConfig.ListIncludeColumns) > 0 {
			if slices.Contains(optConfig.ListIncludeColumns, "context") {
				fields = append(fields, &i.Context)
			}
		} else if len(optConfig.ListExcludeColumns) > 0 {
			if !slices.Contains(optConfig.ListExcludeColumns, "context") {
				fields = append(fields, &i.Context)
			}
		} else {
			fields = append(fields, &i.Context)
		...
		if len(optConfig.ListIncludeColumns) > 0 {
			if slices.Contains(optConfig.ListIncludeColumns, "provider") {
				fields = append(fields, &i.Provider)
			}
		} else if len(optConfig.ListExcludeColumns) > 0 {
			if !slices.Contains(optConfig.ListExcludeColumns, "provider") {
				fields = append(fields, &i.Provider)
			}
		} else {
			fields = append(fields, &i.Provider)
		...
		if err := rows.Scan(fields...); err != nil {
			m.monitoring.Emit(monitoring.EmitRequest{
				ActionIdentifier: "list_ai_usage_scan",
				Message:          "error in scanning rows for ListAiUsage",
				EntityIdentifier: "ai_usage",
	...
	if err := rows.Close(); err != nil {
		m.monitoring.Emit(monitoring.EmitRequest{
			ActionIdentifier: "list_ai_usage_rows",
			Message:          "error closing rows for ListAiUsage",
			EntityIdentifier: "ai_usage",
			Layer:            monitoring.ModuleServiceLayer,
			Type:             monitoring.EmitTypeError,
			Data:             request,
	...
	if err := rows.Err(); err != nil {
		m.monitoring.Emit(monitoring.EmitRequest{
			ActionIdentifier: "list_ai_usage_rows",
			Message:          "error in rows for ListAiUsage",
			EntityIdentifier: "ai_usage",
			Layer:            monitoring.ModuleServiceLayer,
			Type:             monitoring.EmitTypeError,
			Data:             request,
	...
	if err != nil {
		m.monitoring.Emit(monitoring.EmitRequest{
			ActionIdentifier: "list_ai_usage_next",
			Message:          "error determining if list has next page for ListAiUsage",
			EntityIdentifier: "ai_usage",
			Layer:            monitoring.ModuleServiceLayer,
			Type:             monitoring.EmitTypeError,
			Data:             request,
	...
	m.monitoring.Emit(monitoring.EmitRequest{
		...
		Message:          "successfully handled ListAiUsage",
		EntityIdentifier: "ai_usage",
		Layer:            monitoring.ModuleServiceLayer,
	...
	return types.ListResponse{
		AiUsages:    mapModelsToEntities(items),
...
func (m *module) ListCount(ctx context.Context,
	...
	query, err := m.repository.BuildListEntityQuery(ctx, request, main_entity.AiUsage{}, true, nil, nil)
	if err != nil {
		return -1, err
	}
