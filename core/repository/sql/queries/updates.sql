...
SET
version = ?, title = ?, description = ?, project_version_uuid = ?, change_type = ?, data_changes = ?, version_changes = ?, reviews = ?, review_status = ?, owner_uuid = ?, status = ?, created_at = ?, updated_at = ?, created_by_uuid = ?, updated_by_uuid = ?
WHERE uuid = ?;
