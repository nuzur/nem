...
SELECT uuid, version, title, description, review_type, ref_uuid, old_data, old_data_ref, new_data, new_data_ref, reviews, owner_uuid, status, created_at, updated_at, created_by_uuid, updated_by_uuid FROM change_request
WHERE 
     status = ?  
    LIMIT ?, ?
`
...
SELECT uuid, version, name, domains, admin_uuids, memberships, status, created_at, updated_at, created_by_uuid, updated_by_uuid FROM organization
WHERE 
     status = ?  
    LIMIT ?, ?
`
...
SELECT uuid, version, name, enviorments, review_configs, memberships, stores, connections, object_stores, organization_uuid, default_entity, status, created_at, updated_at, created_by_uuid, updated_by_uuid FROM team
WHERE 
     status = ?  
    LIMIT ?, ?
`
