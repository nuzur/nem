// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.8
// 	protoc        v5.29.3
// source: extension_execution.proto

package gen

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	timestamppb "google.golang.org/protobuf/types/known/timestamppb"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type ExtensionExecutionStatus int32

const (
	ExtensionExecutionStatus_EXTENSION_EXECUTION_STATUS_INVALID    ExtensionExecutionStatus = 0
	ExtensionExecutionStatus_EXTENSION_EXECUTION_STATUS_INPROGRESS ExtensionExecutionStatus = 1
	ExtensionExecutionStatus_EXTENSION_EXECUTION_STATUS_SUCCEEDED  ExtensionExecutionStatus = 2
	ExtensionExecutionStatus_EXTENSION_EXECUTION_STATUS_FAILED     ExtensionExecutionStatus = 3
	ExtensionExecutionStatus_EXTENSION_EXECUTION_STATUS_CANCELLED  ExtensionExecutionStatus = 4
)

// Enum value maps for ExtensionExecutionStatus.
var (
	ExtensionExecutionStatus_name = map[int32]string{
		0: "EXTENSION_EXECUTION_STATUS_INVALID",
		1: "EXTENSION_EXECUTION_STATUS_INPROGRESS",
		2: "EXTENSION_EXECUTION_STATUS_SUCCEEDED",
		3: "EXTENSION_EXECUTION_STATUS_FAILED",
		4: "EXTENSION_EXECUTION_STATUS_CANCELLED",
	}
	ExtensionExecutionStatus_value = map[string]int32{
		"EXTENSION_EXECUTION_STATUS_INVALID":    0,
		"EXTENSION_EXECUTION_STATUS_INPROGRESS": 1,
		"EXTENSION_EXECUTION_STATUS_SUCCEEDED":  2,
		"EXTENSION_EXECUTION_STATUS_FAILED":     3,
		"EXTENSION_EXECUTION_STATUS_CANCELLED":  4,
	}
)

func (x ExtensionExecutionStatus) Enum() *ExtensionExecutionStatus {
	p := new(ExtensionExecutionStatus)
	*p = x
	return p
}

func (x ExtensionExecutionStatus) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (ExtensionExecutionStatus) Descriptor() protoreflect.EnumDescriptor {
	return file_extension_execution_proto_enumTypes[0].Descriptor()
}

func (ExtensionExecutionStatus) Type() protoreflect.EnumType {
	return &file_extension_execution_proto_enumTypes[0]
}

func (x ExtensionExecutionStatus) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use ExtensionExecutionStatus.Descriptor instead.
func (ExtensionExecutionStatus) EnumDescriptor() ([]byte, []int) {
	return file_extension_execution_proto_rawDescGZIP(), []int{0}
}

type ExtensionExecution struct {
	state                protoimpl.MessageState   `protogen:"open.v1"`
	Uuid                 string                   `protobuf:"bytes,1,opt,name=uuid,proto3" json:"uuid,omitempty"`
	ExtensionUuid        string                   `protobuf:"bytes,2,opt,name=extension_uuid,json=extensionUuid,proto3" json:"extension_uuid,omitempty"`
	ExtensionVersionUuid string                   `protobuf:"bytes,3,opt,name=extension_version_uuid,json=extensionVersionUuid,proto3" json:"extension_version_uuid,omitempty"`
	ProjectExtensionUuid string                   `protobuf:"bytes,4,opt,name=project_extension_uuid,json=projectExtensionUuid,proto3" json:"project_extension_uuid,omitempty"`
	ProjectUuid          string                   `protobuf:"bytes,5,opt,name=project_uuid,json=projectUuid,proto3" json:"project_uuid,omitempty"`
	ProjectVersionUuid   string                   `protobuf:"bytes,6,opt,name=project_version_uuid,json=projectVersionUuid,proto3" json:"project_version_uuid,omitempty"`
	ExecutedByUuid       string                   `protobuf:"bytes,7,opt,name=executed_by_uuid,json=executedByUuid,proto3" json:"executed_by_uuid,omitempty"`
	Metadata             string                   `protobuf:"bytes,8,opt,name=metadata,proto3" json:"metadata,omitempty"`
	Status               ExtensionExecutionStatus `protobuf:"varint,9,opt,name=status,proto3,enum=nem.ExtensionExecutionStatus" json:"status,omitempty"`
	StatusMsg            string                   `protobuf:"bytes,10,opt,name=status_msg,json=statusMsg,proto3" json:"status_msg,omitempty"`
	CreatedAt            *timestamppb.Timestamp   `protobuf:"bytes,11,opt,name=created_at,json=createdAt,proto3" json:"created_at,omitempty"`
	UpdatedAt            *timestamppb.Timestamp   `protobuf:"bytes,12,opt,name=updated_at,json=updatedAt,proto3" json:"updated_at,omitempty"`
	unknownFields        protoimpl.UnknownFields
	sizeCache            protoimpl.SizeCache
}

func (x *ExtensionExecution) Reset() {
	*x = ExtensionExecution{}
	mi := &file_extension_execution_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ExtensionExecution) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ExtensionExecution) ProtoMessage() {}

func (x *ExtensionExecution) ProtoReflect() protoreflect.Message {
	mi := &file_extension_execution_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ExtensionExecution.ProtoReflect.Descriptor instead.
func (*ExtensionExecution) Descriptor() ([]byte, []int) {
	return file_extension_execution_proto_rawDescGZIP(), []int{0}
}

func (x *ExtensionExecution) GetUuid() string {
	if x != nil {
		return x.Uuid
	}
	return ""
}

func (x *ExtensionExecution) GetExtensionUuid() string {
	if x != nil {
		return x.ExtensionUuid
	}
	return ""
}

func (x *ExtensionExecution) GetExtensionVersionUuid() string {
	if x != nil {
		return x.ExtensionVersionUuid
	}
	return ""
}

func (x *ExtensionExecution) GetProjectExtensionUuid() string {
	if x != nil {
		return x.ProjectExtensionUuid
	}
	return ""
}

func (x *ExtensionExecution) GetProjectUuid() string {
	if x != nil {
		return x.ProjectUuid
	}
	return ""
}

func (x *ExtensionExecution) GetProjectVersionUuid() string {
	if x != nil {
		return x.ProjectVersionUuid
	}
	return ""
}

func (x *ExtensionExecution) GetExecutedByUuid() string {
	if x != nil {
		return x.ExecutedByUuid
	}
	return ""
}

func (x *ExtensionExecution) GetMetadata() string {
	if x != nil {
		return x.Metadata
	}
	return ""
}

func (x *ExtensionExecution) GetStatus() ExtensionExecutionStatus {
	if x != nil {
		return x.Status
	}
	return ExtensionExecutionStatus_EXTENSION_EXECUTION_STATUS_INVALID
}

func (x *ExtensionExecution) GetStatusMsg() string {
	if x != nil {
		return x.StatusMsg
	}
	return ""
}

func (x *ExtensionExecution) GetCreatedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.CreatedAt
	}
	return nil
}

func (x *ExtensionExecution) GetUpdatedAt() *timestamppb.Timestamp {
	if x != nil {
		return x.UpdatedAt
	}
	return nil
}

var File_extension_execution_proto protoreflect.FileDescriptor

const file_extension_execution_proto_rawDesc = "" +
	"\n" +
	"\x19extension_execution.proto\x12\x03nem\x1a\x1fgoogle/protobuf/timestamp.proto\"\xa2\x04\n" +
	"\x12ExtensionExecution\x12\x12\n" +
	"\x04uuid\x18\x01 \x01(\tR\x04uuid\x12%\n" +
	"\x0eextension_uuid\x18\x02 \x01(\tR\rextensionUuid\x124\n" +
	"\x16extension_version_uuid\x18\x03 \x01(\tR\x14extensionVersionUuid\x124\n" +
	"\x16project_extension_uuid\x18\x04 \x01(\tR\x14projectExtensionUuid\x12!\n" +
	"\fproject_uuid\x18\x05 \x01(\tR\vprojectUuid\x120\n" +
	"\x14project_version_uuid\x18\x06 \x01(\tR\x12projectVersionUuid\x12(\n" +
	"\x10executed_by_uuid\x18\a \x01(\tR\x0eexecutedByUuid\x12\x1a\n" +
	"\bmetadata\x18\b \x01(\tR\bmetadata\x125\n" +
	"\x06status\x18\t \x01(\x0e2\x1d.nem.ExtensionExecutionStatusR\x06status\x12\x1d\n" +
	"\n" +
	"status_msg\x18\n" +
	" \x01(\tR\tstatusMsg\x129\n" +
	"\n" +
	"created_at\x18\v \x01(\v2\x1a.google.protobuf.TimestampR\tcreatedAt\x129\n" +
	"\n" +
	"updated_at\x18\f \x01(\v2\x1a.google.protobuf.TimestampR\tupdatedAt*\xe8\x01\n" +
	"\x18ExtensionExecutionStatus\x12&\n" +
	"\"EXTENSION_EXECUTION_STATUS_INVALID\x10\x00\x12)\n" +
	"%EXTENSION_EXECUTION_STATUS_INPROGRESS\x10\x01\x12(\n" +
	"$EXTENSION_EXECUTION_STATUS_SUCCEEDED\x10\x02\x12%\n" +
	"!EXTENSION_EXECUTION_STATUS_FAILED\x10\x03\x12(\n" +
	"$EXTENSION_EXECUTION_STATUS_CANCELLED\x10\x04B9\n" +
	"\x14github.com/nuzur/nemB\x12ExtensionExecutionP\x01Z\vnem/idl/genb\x06proto3"

var (
	file_extension_execution_proto_rawDescOnce sync.Once
	file_extension_execution_proto_rawDescData []byte
)

func file_extension_execution_proto_rawDescGZIP() []byte {
	file_extension_execution_proto_rawDescOnce.Do(func() {
		file_extension_execution_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_extension_execution_proto_rawDesc), len(file_extension_execution_proto_rawDesc)))
	})
	return file_extension_execution_proto_rawDescData
}

var file_extension_execution_proto_enumTypes = make([]protoimpl.EnumInfo, 1)
var file_extension_execution_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_extension_execution_proto_goTypes = []any{
	(ExtensionExecutionStatus)(0), // 0: nem.ExtensionExecutionStatus
	(*ExtensionExecution)(nil),    // 1: nem.ExtensionExecution
	(*timestamppb.Timestamp)(nil), // 2: google.protobuf.Timestamp
}
var file_extension_execution_proto_depIdxs = []int32{
	0, // 0: nem.ExtensionExecution.status:type_name -> nem.ExtensionExecutionStatus
	2, // 1: nem.ExtensionExecution.created_at:type_name -> google.protobuf.Timestamp
	2, // 2: nem.ExtensionExecution.updated_at:type_name -> google.protobuf.Timestamp
	3, // [3:3] is the sub-list for method output_type
	3, // [3:3] is the sub-list for method input_type
	3, // [3:3] is the sub-list for extension type_name
	3, // [3:3] is the sub-list for extension extendee
	0, // [0:3] is the sub-list for field type_name
}

func init() { file_extension_execution_proto_init() }
func file_extension_execution_proto_init() {
	if File_extension_execution_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_extension_execution_proto_rawDesc), len(file_extension_execution_proto_rawDesc)),
			NumEnums:      1,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_extension_execution_proto_goTypes,
		DependencyIndexes: file_extension_execution_proto_depIdxs,
		EnumInfos:         file_extension_execution_proto_enumTypes,
		MessageInfos:      file_extension_execution_proto_msgTypes,
	}.Build()
	File_extension_execution_proto = out.File
	file_extension_execution_proto_goTypes = nil
	file_extension_execution_proto_depIdxs = nil
}
