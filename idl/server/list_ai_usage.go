...
func BuildListAiUsageRequest(ctx context.Context, request *pb.ListAiUsageRequest) (types.ListRequest, *pagination.PageToken, error) {
	err := validatePageSizeForListAiUsage(request)
	if err != nil {
...
func (s *server) ListAiUsage(ctx context.Context, request *pb.ListAiUsageRequest) (*pb.ListAiUsageResponse, error) {

	var err error
	req, pageToken, err := BuildListAiUsageRequest(ctx, request)
	if err != nil {
	...
	if request.GetSkipCache() {
		result, err = s.core.AiUsage().List(ctx, req, ai_usagemodule.WithSkipCache())
	} else {
	...
	// Build the response.
	response := &pb.ListAiUsageResponse{
		AiUsage: pbmapper.AiUsageSliceToProto(result.AiUsage),
	}
...
func validatePageSizeForListAiUsage(request *pb.ListAiUsageRequest) error {
